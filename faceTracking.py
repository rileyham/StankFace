...
def get_face_values(frame, face_mesh):

    frame_rgb = cv.cvtColor(frame, cv.COLOR_BGR2RGB)
    face_detected = face_mesh.process(frame_rgb)
    
    if not face_detected.multi_face_landmarks:
    ...
    return normalized_mouth_openness, head_tilt, normalized_brow_raise

def run_face_tracking_debug(cam, face_mesh):
    ...
    while cam.isOpened():
        ...
        normalized_mouth_openness, head_tilt, normalized_brow_raise = get_face_values(frame, face_mesh)
